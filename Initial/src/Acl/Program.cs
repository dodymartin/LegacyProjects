...
namespace Acl
{
    ...
    {
        private const string LegacyDatabaseConnectionString = @"Server=(localdb)\MSSQLLocalDB;Database=PackageDelivery;Trusted_Connection=true;";
        private const string BubbleDatabaseConnectionString = @"Server=(localdb)\MSSQLLocalDB;Database=PackageDeliveryNew;Trusted_Connection=true;";

        private static readonly TimeSpan IntervalBetweenDeliverySyncs = TimeSpan.FromSeconds(1);
        private static readonly TimeSpan IntervalBetweenProductSyncs = TimeSpan.FromSeconds(1);
        ...
        static void Main(string[] args)
        {
            ...
            _deliverySyncThread = new Task(() => Sync(_deliverySyncronizer.Sync, IntervalBetweenDeliverySyncs), TaskCreationOptions.LongRunning);
            _deliverySyncThread.Start();

            _productSyncronizer = new ProductSyncronizer();
            _productSyncThread = new Task(() => Sync(_productSyncronizer.Sync, IntervalBetweenProductSyncs), TaskCreationOptions.LongRunning);
