...
namespace Acl
{
    partial class Program
    {
        ...
        {
            private readonly string _legacyDatabaseConnectionString;
            private readonly string _bubbleDatabaseConnectionString;

            public DeliverySyncronizer(string legacyDatabaseConnectionString, string bubbleDatabaseConnectionString)
            {
                _legacyDatabaseConnectionString = legacyDatabaseConnectionString;
                _bubbleDatabaseConnectionString = bubbleDatabaseConnectionString;
            }
            ...
            private void SaveLegacyDeliveries(List<DeliveryLegacy> legacyDeliveries)
            {
                using (var connection = new SqlConnection(_legacyDatabaseConnectionString))
                {
                    string query = @"
                        UPDATE [dbo].[DLVR_TBL]
                        SET PRD_LN_1 = @PRD_LN_1, PRD_LN_1_AMN = @PRD_LN_1_AMN,
                            PRD_LN_2 = @PRD_LN_2, PRD_LN_2_AMN = @PRD_LN_2_AMN,
                            ESTM_CLM = @ESTM_CLM, STS = 'R'
                        ...
                        BEGIN
                            ...
                            SET 
                            PRD_LN_3 = @PRD_LN_3, PRD_LN_3_AMN = @PRD_LN_3_AMN,
                            PRD_LN_4 = @PRD_LN_4, PRD_LN_4_AMN = @PRD_LN_4_AMN
                        WHERE NMB_CLM = @NMB_CLM
                        END
                        ELSE    
                        BEGIN
            ...
            {
                using (var connection = new SqlConnection(_bubbleDatabaseConnectionString))
                {
                    string query = @"
            ...
            private List<DeliveryLegacy> ReadUpdatedLegacyDeliveries()
            {
                using (var connection = new SqlConnection(_legacyDatabaseConnectionString))
                {
                    string query = @"
                        ...
                        INNER JOIN [dbo].[ADDR_TBL] a ON d.NMB_CLM = a.DLVR
                        WHERE d.IsSyncNeeded = 1
                    ...
                    return connection.Query<DeliveryLegacy>(query).ToList();
                }
            }
